...
class AIEngine:
    ...
    async def analyze(self, text: str) -> AnalysisResult:
        ...
        # Analysis status: the heuristic engine always ran, so this is either a
        # full analysis or a partial one (LLM unavailable). It is never
        # "unavailable"/"failed" from the server side — that state is reserved
        # for client-side connectivity failures.
        analysis_status = "analyzed" if gemini_result is not None else "partial"
