...
class Scan(Base):
    ...
    # Fingerprint of the X-Device-Token / API key that created this scan.
    # Enables per-device ownership: a device may only read/delete/history its
    # own scans. NULL when API_AUTH_ENABLED=false (trusted-network mode).
...
class CachedOSINT(Base):
    ...
    indicator = Column(String, index=True) # "hash", "url", "ip"
    type = Column(String) # "file", "domain", "ip"
    data = Column(JSON)
