...
def esc(value: Any) -> str:
    """Escape a value for safe interpolation into ReportLab Paragraph markup.

    Paragraph text is parsed as a small XML/HTML-like markup subset; unescaped
    '&', '<', '>' in attacker/LLM-controlled fields (AI explanations, package
    names, YARA rule names, ...) makes reportlab raise on malformed markup,
    which silently aborts PDF generation for the whole report.
    """
...
class ReportGenerator:
    ...
    def generate_pdf(self, report_id: str, analysis_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            elements.append(Paragraph(f"<b>Risk Level:</b> {esc(risk_info.get('level', 'UNKNOWN'))}", styles['Normal']))
            elements.append(Paragraph(f"<b>Risk Score:</b> {esc(risk_info.get('score', 0))}", styles['Normal']))
            ...
            elements.append(Paragraph(esc(ai_explanation), styles['Normal']))
            elements.append(Spacer(1, 12))

            # Package Info & Hashes
            elements.append(Paragraph("Application Info", styles['Heading1']))
            andro_info = analysis_data.get("androguard", {})
            elements.append(Paragraph(f"<b>Package:</b> {esc(andro_info.get('package', 'Unknown'))}", styles['Normal']))
            elements.append(Paragraph(f"<b>Version:</b> {esc(andro_info.get('version_name', 'Unknown'))} ({esc(andro_info.get('version_code', 'Unknown'))})", styles['Normal']))
            elements.append(Paragraph(f"<b>SHA256:</b> {esc(analysis_data.get('hash', 'Unknown'))}", styles['Normal']))
            elements.append(Spacer(1, 12))
            ...
            if permissions:
                for perm in permissions[:10]: # Limit to 10 for brevity in PDF
                    elements.append(Paragraph(f"- {esc(perm)}", styles['Normal']))
            ...
            elements.append(Paragraph(f"Status: {esc(mobsf_status)}", styles['Normal']))
            if mobsf_status == "success":
                mobsf_score = mobsf_info.get("report", {}).get("security_score", "Unknown")
                elements.append(Paragraph(f"Security Score: {esc(mobsf_score)}", styles['Normal']))
            elements.append(Spacer(1, 12))
            ...
            if yara_matches:
                for match in yara_matches:
                    elements.append(Paragraph(f"- Rule matched: {esc(match.get('rule'))}", styles['Normal']))
            else:
            ...
            if osint_info:
                elements.append(Paragraph(f"Malicious: {esc(osint_info.get('malicious', 0))}", styles['Normal']))
                elements.append(Paragraph(f"Suspicious: {esc(osint_info.get('suspicious', 0))}", styles['Normal']))
                elements.append(Paragraph(f"Undetected: {esc(osint_info.get('undetected', 0))}", styles['Normal']))
            else:
            ...
            if risk_info.get("details"):
                for detail in risk_info.get("details", []):
                    elements.append(Paragraph(f"- <b>Risk Factor:</b> {esc(detail)}", styles['Normal']))
            else:
