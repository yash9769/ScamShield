...
/// Whether the message was actually analysed.
///
/// A result with status != [AnalysisStatus.analyzed] must NEVER be rendered
/// as a green "Safe" verdict — "we could not check" is not the same as
/// "verified safe".
enum AnalysisStatus {
  /// The engine produced a genuine verdict (locally or via backend).
  analyzed,

  /// Only part of the pipeline ran (e.g. LLM down, heuristic fallback used).
  partial,

  /// The analysis service could not be reached; nothing was checked.
  unavailable,

  /// The analysis service was reached but returned an error.
  failed,
}
...
class AnalysisResult {
  ...
  /// Whether this represents a real verdict. See [AnalysisStatus].
  ...
  /// True when this result is a real verdict (never render failed/unavailable
  /// results as safety).
  bool get isAnalyzed => status == AnalysisStatus.analyzed || status == AnalysisStatus.partial;
}
...
class ScamDetector {
  ...
  static AnalysisResult analyzeApk(ApkAnalysisResult apk, List<OsintResult> osintResults) {
    ...
    // 3.6 Potential capability indicators (structural feature fingerprint)
    // Terminology is deliberate: a string reference inside DEX bytes indicates
    // a capability MAY exist — it is not proof that the code executes it.
    if (apk.dangerousApis.isNotEmpty) {
      final apiScore = (apk.dangerousApis.length * 12).clamp(0, 40);
      score += apiScore;
      reasons.add(DetectionReason(
        label: 'Potential Capability Indicators',
        description:
            'App code references sensitive capabilities: ${apk.dangerousApis.join(', ')}. '
            'References may indicate abuse, but legitimate apps can use them too — '
